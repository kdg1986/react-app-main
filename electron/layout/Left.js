...
const menu = [
  ...
  { code: 'RMS', title: 'electron 시스템관리', depth: 0, no: 3 },
...
export default () => {
      ...
      if (subData.length > 0) {
        acc = acc.concat(
          <SubMenu key={cur.code} icon={<UserOutlined />} title={cur.title}>
            {createMenu(
              subData.map(item => {
                item.namePath = `${item.namePath || item.title},${cur.title}`;
                return item;
              }),
            )}
          </SubMenu>,
        ...
        acc = acc.concat(
          <Menu.Item key={`${cur.code}_${cur.no}`} onClick={() => handleClick(cur)}>
            {cur.title}
          </Menu.Item>,
        );
      }
  ...
  useEffect(() => {
    const _menu = DEVELOP_MODE
      ? menu
          .filter(item => item.depth === 0)
          .concat([{ code: 'SAMPLE', title: '예제', depth: 0, no: 6, path: '/sample' }])
