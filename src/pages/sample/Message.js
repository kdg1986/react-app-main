...
export default () => {    
    return(
        <>
            <PageHeader
                ...
                onBack={() => history.back()}
                title={[ "Message", <Button key={keygen} type="link" href="https://ant.design/components/message/#API" target="_blank">API</Button> ]}
            />            
            <Typo>{"import { message } from 'antd';"}</Typo><p/>
    ...
    duration : 0.5,
    onClose : () => console.log("Message")
})}>Message</Button>`}</pre></Typo></>} />

            <Addrow
            title="error"
            divider={true}
            demo={<>
                <Button type="primary" onClick={()=>message.error({
                    content : "error",
                    duration : 0.5,
    ...
    duration : 0.5,
    onClose : () => console.log("error")
})}>error</Button>`}</pre></Typo></>} />

            <Addrow
            title="info"
            divider={true}
            demo={<>
                <Button type="primary" onClick={()=>message.info({
                    content : "Message",
                    duration : 0.5,
    ...
    duration : 0.5,
    onClose : () => console.log("info")
})}>info</Button>`}</pre></Typo></>} />
            
            <Addrow
            title="warn"
            divider={true}
            demo={<>
                <Button type="primary" onClick={()=>message.warn({
                    content : "warn",
                    duration : 0.5,
    ...
    duration : 0.5,
    onClose : () => console.log("warn")
})}>warn</Button>`}</pre></Typo></>} />

            <Addrow
            title="loading"
            divider={true}
            demo={<>
                <Button type="primary" onClick={()=>message.loading({
                    content : "loading",
                    duration : 0.5,
    ...
    onClose : () => console.log("loading")
            ...
            code={<><Typo codeStyle={false}>
                <pre>{`<Button type="primary" onClick={()=>message.destroy()}>destroy</Button>`}</pre></Typo></>} />
